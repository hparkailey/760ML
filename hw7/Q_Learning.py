...
class QLearning(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for step_i in range(self.options.steps):
    ...
    def epsilon_greedy_action(self, state):
        ...
        #   YOUR IMPLEMENTATION HERE   #
        ################################
        action_probs = np.zeros(self.env.action_space.n) + (self.options.epsilon/(self.env.action_space.n - 1))
        q_val = self.Q[state]
        best_action = np.argmax(q_val)
        action_probs[best_action] = 1 - self.options.epsilon
        return action_probs
