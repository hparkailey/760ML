...
class ValueIteration(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for each_state in range(self.env.nS):
            ...
            action = policy(each_state) #returns best action given a state

            #probability : prob of next state and reward given current state and action
            # r: reward taking action in the current state 


            for probability, next_state, reward, done in self.env.P[each_state][action]:
                self.V[each_state] = sum([probability*(reward + self.options.gamma*self.V[next_state])])
    ...
    def create_greedy_policy(self):
        ...
        def policy_fn(state):
            ...
            for action in range(self.env.nA):
                e_val = 0
                for probability, next_state, reward, done in self.env.P[state][action]:
                    e_val += sum([probability*(reward + self.options.gamma*self.V[next_state])])
                expected_vals.append(e_val)
            best_action = np.argmax(expected_vals)
